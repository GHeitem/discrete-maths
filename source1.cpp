...
int func(t_point a, t_point b)
{
	return a.x%b.x == 0;
}
int ff(t_point a, t_point b){
	return (a.x <= b.x) && (a.y <= b.y);
}
...
//	//system("pause");
//}
const t_point M11[] = {  2,0, 3,0, 6,0 ,12,0};
void test2() {
	relation a = NULL;
	getMem(&a, 6);
	getRel(a, 6, triangle, ff);
	determine(chRel(a, 6));
	t_point res;
	int q=1;
	Get_naib(a, 6, triangle, &res);
	printf("\nHow many: %d\n", q);
	putPoint(res);
	dominate(a,6,a);
	printRel(a,6,triangle);
