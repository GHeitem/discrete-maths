...
int chRel(relation a, int N)
{
	...
	printf("Reflex: %s\n", reflex(a, N) ? "true" : "false");
	printf("Simmetry: %s\n", simmetry(a, N) ? "true" : "false");
	printf("Transitivity: %s\n", transitivity(a, N) ? "true" : "false");
	printf("Assimmetry: %s\n", assimetry(a, N) ? "true" : "false");
	printf("Full: %s\n", full(a, N) ? "true" : "false");
	printf("Areflex: %s\n", areflex(a, N) ? "true" : "false");
...
void makeNull(int a[], int size){
	for (int i=0; i<size; i++){
		a[i]=0;
	};
};
void topological(relation a, int size, int levels[])
{
	int l=1;
	makeNull(levels, size);
	for (int j=0; j<size; j++){
		int flag=0;
		if (!levels[j]){
...
void Get_naib(relation A, int size, const t_point *M, t_point *res){
	int i, flag = 1;
	for (i = 0; i < size; i++){
		flag = 1;
...
void Get_naim(relation A, int size, const t_point *M, t_point *res){
	int i, flag = 1;
	for (i = 0; i < size; i++){
		flag = 1;
		for (int j = 0; (j < size)&&flag; j++){
...
void Get_min(relation A, int size, const t_point *M, t_point *res,int *q){
	int k=0,i,j;
	for (i = 0; i < size; i++){
		for ( j = 0; j < size; j++){
			if (A[j][i]&&!A[i][j]){
...
void Get_max(relation A, int size, const t_point *M, t_point *res,int *q){
	int k = 0, i, j;
	for (i = 0; i < size; i++){
		for (j = 0; j < size; j++){
			if (!A[j][i] && A[i][j]){
